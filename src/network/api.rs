 ...
 */
pub async fn write(mut payload: Payload) -> HandlerResponse {
...
pub async fn read(mut payload: Payload) -> HandlerResponse {
    let mut bytes = BytesMut::new();
    while let Some(item) = ntex::util::stream_recv(&mut payload).await {
        bytes.extend_from_slice(&item.unwrap());
    }
...
pub async fn consistent_read(mut payload: Payload) -> HandlerResponse {
    let ret = RAFT_CLIENT.get().unwrap().ensure_linearizable().await;
